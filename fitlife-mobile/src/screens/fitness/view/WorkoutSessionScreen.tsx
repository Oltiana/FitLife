...
export function WorkoutSessionScreen() {
  ...
  const handleCompleteWorkout = async () => {
    ...
    const durationMinutes = startedAt
      ? Math.max(1, Math.round((new Date().getTime() - startedAt.getTime()) / 60000))
      : 1;
    try {
      await completeWorkoutSession(activeSessionId, { durationMinutes, calories: 0 });
  ...
  const handleRemoveExercise = (exerciseId: number) => {
    Alert.alert('Remove Exercise', 'Are you sure you want to remove this exercise from the plan?', [
  ...
  const handleUpdateExercise = async () => {
    ...
    try {
      await updateWorkoutExercise(selectedExercise.id, { sets: Number(editSets), reps: Number(editReps) });
      await loadPlan();
            ...
            <View style={styles.setRow}>
              {Array.from({ length: currentExercise.sets ?? 1 }).map((_, index) => (
                <Pressable
                  ...
                  onPress={() => { setCurrentSet(index + 1); setCurrentExerciseIndex(0); setSecondsLeft(84); setIsPaused(false); }}
                >
...
const styles = StyleSheet.create({
  ...
  center: { flex: 1, backgroundColor: '#F8FAF7', alignItems: 'center', justifyContent: 'center' },
  loadingText: { marginTop: 10, color: '#5F8F64', fontWeight: '700' },
  errorText: { color: '#B00020', fontWeight: '800' },
  header: { backgroundColor: '#86B587', paddingHorizontal: 22, paddingTop: 34, paddingBottom: 24, flexDirection: 'row', alignItems: 'center' },
  backButton: { width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(255,255,255,0.25)', alignItems: 'center', justifyContent: 'center', marginRight: 16 },
  headerCenter: { flex: 1, alignItems: 'center', marginRight: 60 },
  title: { color: '#fff', fontSize: 22, fontWeight: '900', textTransform: 'capitalize', textAlign: 'center' },
  subtitle: { color: '#F1FFF2', marginTop: 4, fontWeight: '800', textAlign: 'center' },
  content: { flex: 1 },
  scrollContent: { padding: 22, paddingBottom: 170 },
  timerCard: { backgroundColor: '#6F9B73', borderRadius: 22, paddingVertical: 24, alignItems: 'center', marginBottom: 20 },
  timerText: { color: '#E8A22D', fontSize: 42, fontWeight: '900' },
  timerLabel: { color: '#EAF4E8', fontSize: 13, fontWeight: '900', marginTop: 2 },
  progressHeader: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 },
  progressLabel: { color: '#5F8F64', fontSize: 15, fontWeight: '900' },
  progressPercent: { color: '#5F8F64', fontSize: 15, fontWeight: '900' },
  progressBackground: { height: 10, backgroundColor: '#DCEADB', borderRadius: 10, overflow: 'hidden', marginBottom: 22 },
  progressFill: { height: '100%', backgroundColor: '#5F8F64', borderRadius: 10 },
  ...
  currentAccent: { width: 28, height: 76, borderRadius: 14, backgroundColor: '#6F9B73', marginRight: 14 },
  currentExerciseContent: { flex: 1 },
  activeLabel: { color: '#9A9A9A', fontSize: 12, fontWeight: '900' },
  activeTitle: { color: '#245C32', fontSize: 19, fontWeight: '900', marginTop: 3, textTransform: 'capitalize' },
  activeMeta: { color: '#5F8F64', fontSize: 14, fontWeight: '800', marginTop: 3 },
  ...
  setCircleActive: { backgroundColor: '#86B587', borderColor: '#86B587' },
  ...
  pauseButton: { width: 96, height: 96, borderRadius: 48, backgroundColor: '#E8A22D', alignItems: 'center', justifyContent: 'center' },
  nextControlButton: { width: 70, height: 70, borderRadius: 35, backgroundColor: '#DCEADB', borderWidth: 1, borderColor: '#AFC9AF', alignItems: 'center', justifyContent: 'center' },
  ...
  sectionTitle: { color: '#5F8F64', fontSize: 22, fontWeight: '900', marginBottom: 14 },
  ...
  exerciseName: { color: '#5F8F64', fontSize: 16, fontWeight: '800', textTransform: 'capitalize' },
  exerciseMeta: { color: '#7FAE83', fontSize: 13, fontWeight: '700', marginTop: 4, textTransform: 'capitalize' },
  exerciseBadge: { backgroundColor: '#DCEADB', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 6, alignSelf: 'flex-start', marginTop: 10 },
  exerciseBadgeText: { color: '#6F9B73', fontSize: 11, fontWeight: '800' },
  actionButtons: { flexDirection: 'row', gap: 8, marginLeft: 8 },
  editExerciseButton: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#DCEADB', alignItems: 'center', justifyContent: 'center' },
  removeExerciseButton: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#FFE1D0', alignItems: 'center', justifyContent: 'center' },
  statusCard: { backgroundColor: '#DCEADB', borderRadius: 16, padding: 14, marginTop: 10, marginBottom: 14, flexDirection: 'row', alignItems: 'center', gap: 8 },
  statusText: { color: '#5F8F64', fontWeight: '800', flex: 1 },
  startButton: { backgroundColor: '#86B587', paddingVertical: 16, borderRadius: 24, alignItems: 'center', marginTop: 6 },
  completeButton: { backgroundColor: '#5F8F64', paddingVertical: 16, borderRadius: 24, alignItems: 'center', marginTop: 6 },
  ...
  modalTitle: { color: '#5F8F64', fontSize: 22, fontWeight: '800' },
  modalSubtitle: { color: '#777', fontWeight: '700', marginTop: 6, marginBottom: 18, textTransform: 'capitalize' },
  inputLabel: { color: '#5F8F64', fontWeight: '800', marginBottom: 6 },
  ...
  saveButton: { flex: 1, backgroundColor: '#86B587', paddingVertical: 13, borderRadius: 16, alignItems: 'center' },
